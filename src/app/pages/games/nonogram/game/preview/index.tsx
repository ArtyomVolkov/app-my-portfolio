...
const Preview: React.FC<IPreview> = ({ initialDraw = false, className }) => {
  ...
  const resize = useMemo(() => {
    ...
    return { width: data.size[1] * 50, height: data.size[0] * 50 };
  }, [data.size]);
  const drawBox = useMemo(() => {
    if (!data.size) {
      return 0;
    }
    return Math.min(resize.width, resize.height) / Math.min(...data.size);
  }, [data.size]);
  ...
  const draw = () => {
    if (!data.lastActive || !canvasRef.current) {
      return;
    }
  ...
  return (
    <div className={mergeClassNames([styles.preview, className])}>
      <canvas ref={canvasRef} width={resize.width} height={resize.height} />
    </div>
  );
};
