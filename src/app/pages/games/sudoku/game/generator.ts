...
class SudokuGenerator {
  ...
  static setHint = (blank: Matrix, filled: Matrix): Matrix | null => {
    const blocks: Array<Array<number> | null> = [];
    const emptyBlocs: number[] = [];
    const data = [...blank];
    ...
    blank.forEach((row, index) => {
      ...
      blocks[index] = row.reduce((acc: number[], item, i) => {
        if (!item) {
          acc.push(i);
        }
        return acc;
    ...
    const block =
      emptyBlocs[Math.round(Math.random() * (emptyBlocs.length - 1))];
    const cell =
      blocks?.[block]?.[
        Math.round(Math.random() * (blocks[block]?.length - 1))
      ];

    if (block !== undefined && cell !== undefined) {
  ...
  private shuffle = (): Matrix => {
    ...
    const list = Array(SudokuGenerator.size)
      .fill(1)
      .map((_, index) => index + 1);

    Array(SudokuGenerator.size)
      .fill(1)
      .forEach((_, index) => {
  ...
  private getBlocks = (matrix: Matrix): Matrix => {
    const blocks: Matrix = [];
  ...
  private reduce = (data: Matrix, level: Level) => {
    const matrix = data.map((item: Array<number>) => item.slice()); // copy of two dimensional array
